...
- Root folder contains an example Dockerfile.txt for Streamlit demo. A self-contained pre-build docker image can be run with command "docker run -d -p 8503:8501 --gpus all guntisb/clic:commit2". It can serve as a template for the ROS docker integration.
